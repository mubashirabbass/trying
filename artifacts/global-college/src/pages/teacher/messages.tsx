  ...
  return (
    <DashboardLayout>
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Messages</h1>
          <p className="text-sm text-gray-500 mt-1">Private conversations with your students</p></div>
        <Dialog open={newOpen} onOpenChange={setNewOpen}>
          <DialogTrigger asChild>
            <Button size="sm"><Plus className="h-4 w-4 mr-2" /> New Conversation</Button>
          </DialogTrigger>
          <DialogContent className="max-w-sm">
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={startThread} disabled={!newStudentId}>Start</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
      ...
      <div className="flex h-[calc(100vh-220px)] gap-4 min-h-[400px]">
        <div className="w-80 shrink-0 bg-white rounded-xl border border-gray-200 flex flex-col">
          ...
          {loading ? <div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
            : threads.length === 0 ? (
              <div className="text-center py-12 px-4"><MessageSquare className="h-8 w-8 text-gray-400 mx-auto mb-2" />
                <p className="text-sm text-gray-500">No conversations yet.</p></div>
            ) : (
                    ...
                    <button key={t.id} onClick={() => setSelectedThread(t)}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="font-semibold text-sm text-gray-900 truncate">{otherName ?? "Student"}</p>
        ...
        <div className="flex-1 bg-white rounded-xl border border-gray-200 flex flex-col">
          {!selectedThread ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center"><MessageSquare className="h-12 w-12 text-gray-300 mx-auto mb-3" />
                <p className="text-gray-500">Select a conversation</p></div>
            </div>
          ) : (
            <>
              <div className="p-4 border-b flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-sm">{selectedThread.studentName}</p>
              ...
              <form onSubmit={sendMessage} className="p-4 border-t flex gap-3">
                <Input value={body} onChange={e => setBody(e.target.value)} placeholder="Type a message..." className="flex-1" disabled={sending} />
