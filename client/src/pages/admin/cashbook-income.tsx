...
const STORAGE_KEY = "gccs_cashbook_income_v1";
...
function EntryForm({ onAdd, onUpdate, onClose, editEntry }: EntryFormProps) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (editEntry && onUpdate) {
      onUpdate(entry);
      toast({ title: "آمدن اپڈیٹ ہو گئی ✓" });
    } else if (onAdd) {
      onAdd(entry);
      toast({ title: "آمدن شامل ہو گئی ✓" });
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        {/* Header */}
        <div className="px-6 py-4 rounded-t-2xl flex items-center justify-between bg-emerald-600">
          <h3 className="text-white font-bold text-base">
            📥 تفصیل آمدن — {editEntry ? "ترمیم" : "نئی اندراج"}
          </h3>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          ...
          <div>
            <Label className="text-sm font-semibold text-gray-700 mb-1.5 block">آمدن کی تفصیل</Label>
            <Input
              type="text"
              value={desc}
          ...
          <div>
            <Label className="text-sm font-semibold text-gray-700 mb-1.5 block">آمدن کی قسم</Label>
            <select
              value={cat}
          ...
          <div className="flex gap-3 pt-2">
            <Button type="submit" className="flex-1 h-10 text-sm font-bold bg-emerald-600 hover:bg-emerald-700">
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-100 border-b-2 border-gray-300">
              ...
              <th className="border border-gray-300 px-3 py-2 text-sm font-bold text-gray-700">تفصیل آمدن</th>
          ...
          <tbody>
            {entries.length === 0 ? (
              ...
              entries.map((entry, idx) => {
                ...
                return (
                  <tr key={entry.id} className="hover:bg-emerald-50">
                    ...
                    <td className="border border-gray-300 px-3 py-2 text-sm text-right font-mono font-semibold text-emerald-700">
                    ...
                    <td className="border border-gray-300 px-3 py-2 text-sm text-right font-mono font-semibold bg-emerald-50">
                      {runningTotal.rs.toLocaleString()}
                    </td>
                    <td className="border border-gray-300 px-3 py-2 text-sm text-right font-mono bg-emerald-50">
...
export default function CashbookIncome() {
  const { toast } = useToast();
  ...
  const addEntry = (entry: LedgerEntry) => {
    set({ incomeEntries: [...data.incomeEntries, entry] });
  };

  const updateEntry = (entry: LedgerEntry) => {
    set({ 
      incomeEntries: data.incomeEntries.map(e => e.id === entry.id ? entry : e) 
    });
  };

  const removeEntry = (id: string) => {
    if (!window.confirm("کیا آپ واقعی یہ اندراج حذف کرنا چاہتے ہیں؟")) return;
    set({ incomeEntries: data.incomeEntries.filter(e => e.id !== id) });
    toast({ title: "اندراج حذف ہو گیا" });
  };

  const clearAll = () => {
    if (!window.confirm("کیا آپ تمام آمدن کی تفصیلات صاف کرنا چاہتے ہیں؟")) return;
  ...
  const total = getTotal(data.incomeEntries);
  const filteredEntries = filterEntries(data.incomeEntries, filterType, filterDate, data.year, data.month);
  ...
  return (
    <DashboardLayout>
      <div className="pb-12" style={{ fontFamily: "'Noto Nastaliq Urdu', serif" }}>
            ...
            return (
              <div key={pageIndex} className="print-page page-break" style={{ direction: "rtl", fontFamily: "'Noto Nastaliq Urdu', serif" }}>
                ...
                <div className="print-header">
                  ...
                  <div className="border-4 border-emerald-600 rounded-lg p-4 mb-3 bg-gradient-to-r from-emerald-50 to-white">
                    <div className="flex items-center justify-between">
                      {/* Left: Logo/Icon */}
                      <div className="text-center" style={{ width: "15%" }}>
                        <div className="w-16 h-16 rounded-full bg-emerald-600 flex items-center justify-center mx-auto mb-1">
                      ...
                      <div className="text-center" style={{ width: "70%" }}>
                        <h1 className="text-2xl font-black text-emerald-800 mb-1" style={{ fontFamily: "'Noto Nastaliq Urdu', serif" }}>
                  ...
                  {/* Document Title Bar */}
                  <div className="bg-emerald-600 text-white py-2 px-4 mb-2 rounded">
                    <h2 className="text-center text-lg font-bold">آمدن کی کیش بک</h2>
                  </div>
                  
                  {/* Document Info Section */}
                  <div className="grid grid-cols-3 gap-2 mb-2 text-xs border-b-2 border-emerald-300 pb-2">
                ...
                <table className="w-full border-collapse border-2 border-black print-table">
                  <thead>
                    <tr>
                      ...
                      <th className="border-2 border-black px-2 py-2 text-xs font-bold">تفصیل آمدن</th>
                  ...
                  <tfoot>
                    <tr>
                      <td colSpan={3} className="border-2 border-black px-2 py-2 text-xs font-bold bg-gray-100">
                        اس صفحہ کی کل آمدن ({pageEntries.length} اندراجات)
                      </td>
                    ...
                    {pageIndex === printPages.length - 1 && filteredEntries.length > 0 && (
                      <tr>
                        <td colSpan={3} className="border-2 border-black px-2 py-2 text-sm font-bold bg-emerald-100">
                          مجموعی کل آمدن ({filteredEntries.length} اندراجات)
                        </td>
                        <td className="border-2 border-black px-2 py-2 text-sm text-right font-bold bg-emerald-100">
                          {filteredTotal.rs.toLocaleString()}
                        </td>
                        <td className="border-2 border-black px-2 py-2 text-sm text-right font-bold bg-emerald-100">
                          {filteredTotal.ps.toString().padStart(2, "0")}
                        </td>
                        <td colSpan={2} className="border-2 border-black bg-emerald-100"></td>
                      </tr>
                    )}
                  </tfoot>
                </table>
        ...
        {/* Toolbar */}
        <div className="mb-4 bg-emerald-600 border-b-2 border-emerald-800 p-3 flex flex-wrap items-center gap-2 print:hidden sticky top-0 z-10 shadow-lg">
          <Button
            onClick={() => setShowForm(true)}
            className="bg-white text-emerald-700 hover:bg-emerald-50 font-bold h-9 px-4 text-sm"
            style={{ fontFamily: "'Noto Nastaliq Urdu', serif" }}
          >
            <Plus className="h-4 w-4 ml-2" /> 📥 نئی آمدن
          </Button>

          <Link href="/admin/cashbook-expenses">
            <Button
              className="bg-emerald-800 hover:bg-emerald-900 text-white font-bold h-9 px-4 text-sm"
              style={{ fontFamily: "'Noto Nastaliq Urdu', serif" }}
            >
              📤 خرچ کی کیش بک
            </Button>
          ...
          <div className="h-6 w-px bg-emerald-400 mx-1"></div>
        ...
        {showFilters && (
          <div 
            className="mb-4 bg-emerald-100 border border-emerald-300 rounded-lg p-3 print:hidden"
            style={{ direction: "rtl", fontFamily: "'Noto Nastaliq Urdu', serif" }}
          >
            <div className="flex flex-wrap items-center gap-2">
              <Label className="text-xs font-bold text-emerald-800">فلٹر:</Label>
        ...
        {/* Header Section */}
        <div 
          className="bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-300 rounded-lg p-4 mb-4 shadow-sm"
          style={{ direction: "rtl" }}
        >
          <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
            <div className="flex items-center gap-3">
              <div>
                <Label className="text-xs font-semibold text-emerald-700 mb-1 block">سال</Label>
                <Input
                  type="number"
                  value={data.year}
                  onChange={e => set({ year: e.target.value })}
                  className="w-20 h-8 text-right font-bold border-emerald-300 text-sm"
                />
              </div>
              <div>
                <Label className="text-xs font-semibold text-emerald-700 mb-1 block">ماہ</Label>
                <select
                  value={data.month}
                  onChange={e => set({ month: e.target.value })}
                  className="h-8 px-2 border border-emerald-300 rounded-md text-right bg-white font-bold w-24 text-sm"
                >
                  {MONTHS.map(m => (
            ...
            <div className="text-center">
              <h1 className="text-lg font-black text-emerald-900 mb-1">📥 آمدن کی کیش بک</h1>
              <p className="text-xs text-emerald-700 font-semibold">Global College of Computer Science</p>
            </div>
          </div>

          {/* Balance Summary */}
          <div className="mt-3 pt-3 border-t border-emerald-300">
            <div className="bg-emerald-600 border border-emerald-700 rounded-lg p-4 text-center">
              <p className="text-xs font-bold text-emerald-100 uppercase mb-1">
                {filterType === "all" ? "کل آمدن" : 
                 filterType === "today" ? "آج کی آمدن" :
                 filterType === "thisMonth" ? "اس ماہ کی آمدن" :
                 filterType === "selectedDate" ? "منتخب تاریخ کی آمدن" :
                 "منتخب ماہ کی آمدن"}
              </p>
